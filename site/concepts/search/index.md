...
1. It calls the search tool with a natural language query
1. The tool generates an embedding for the query
1. RedisVL performs a vector similarity search against the index
1. The top results are returned to the agent as context
...
In addition to the in-process Python tools, you can connect an agent to RedisVL's own MCP server (one index per server) using ADK's standard `McpToolset` pointed at a running `rvl mcp` instance. The server exposes schema-aware `search-records` and `upsert-records` tools and is useful when the same index needs to be served to multiple agents or non-Python clients. See the [search tools how-to](https://redis-developer.github.io/adk-redis/user_guide/how_to_guides/search_tools/index.md) for the decision matrix and code samples.
